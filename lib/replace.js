...
function replace( str, search, newval ) {
	...
	if ( isString( search ) ) {
		...
		throw new TypeError( format( 'invalid argument. Second argument must be a string primitive or regular expression. Value: `%s`.', search ) );
	}
	if ( !isString( newval ) && !isFunction( newval ) ) {
		throw new TypeError( format( 'invalid argument. Third argument must be a string primitive or replacement function. Value: `%s`.', newval ) );
	}
